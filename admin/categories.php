             ...
             <div class="bs-calltoaction bs-calltoaction-info">
                    <div class="row">
                        <div class="col-md-9 cta-contents">
                            <h1 class="cta-title">There\'s No Categories To Show</h1>
                        </div>
                        <div class="col-md-3 cta-button">
                            <a href="categories.php?do=Add" class="btn btn-lg btn-block btn-info">New Category</a>
                        </div>
                     </div>
              </div>
            </div>
        ...
        <div class="container">
            <form class="form-horizontal" action="?do=Insert" method="POST">
                ...
                <div class="form-group form-group-lg">
                    ...
                    <div class="col-sm-10 col-md-6">
                        <input type="number" name="ordering" class="form-control" placeholder="Number To Arrange The Categories" required/>
                    </div>
                </div>
                <!-- End Ordering Field -->
                <!-- Start Category Type -->
                <div class="form-group form-group-lg">
                    ...
                    <div class="col-sm-10 col-md-6">
                        <select name="parent" title="select of parent category" required>
...
<?
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            // Filter Ordering
            $order = trim($_POST['ordering']);
            $order = strip_tags($order);
            $order = filter_var($order, FILTER_SANITIZE_NUMBER_INT);
            // Filter Visibility
            $visible = trim($_POST['visibility']);
            $visible = strip_tags($visible);
            $visible = filter_var($visible, FILTER_SANITIZE_NUMBER_INT);
            // Filter Commenting
            $comment = trim($_POST['commenting']);
            $comment = strip_tags($comment);
            $comment = filter_var($comment, FILTER_SANITIZE_NUMBER_INT);
            // Filter Ads
            $ads = trim($_POST['ads']);
            $ads = strip_tags($ads);
            $ads = filter_var($ads, FILTER_SANITIZE_NUMBER_INT);
            ...
            if ($check == 1) {
                ...
                $theMsg = '<div class="container"><div class="alert alert-danger">Sorry This Category Is Exist</div></div>';
...
// Check If Get Request catid Is Numeric & Get Its Integer Value
        ...
        if ($count > 0) { ?>
            ...
            $theMsg = '<div class="container"><div class="alert alert-danger">Theres No Such ID</div></div>';
